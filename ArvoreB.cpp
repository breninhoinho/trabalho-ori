...
// Retorna o RRN da chave ou -1, caso não exista
int ArvoreB::buscar(int chave) {
    ...
    while (idx != -1) {
        ...
        // Procura a primeira chave maior ou igual a chave buscada.
        int i = 0;
        ...
        // Se chegou a uma folha, a chave nao existe.
        if (x.folha) 
            return -1;
...
bool ArvoreB::atualizar(int chave, int rrn) {
    ...
    while (idx != -1) {
        ...
        // Encontra a chave, atualiza o RRN e grava novamente no arquivo
        if (i < x.n && x.chaves[i] == chave) {
            x.dados[i] = rrn;
            escreverNo(idx, x);
            return true;
        ...
        // Se chegou em uma folha e nao encontrou a chave, ela nao existe
        if (x.folha) 
            return false;
